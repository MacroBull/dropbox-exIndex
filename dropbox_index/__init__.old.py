...
SCRIPT_WWW = 'https://code.google.com/p/kosciak-misc/wiki/DropboxIndex'

FILES_URL = 'https://dl.dropboxusercontent.com/u/73985358'
	...
	for keys, value in FILE_TYPES.items():
		if filetype in keys:
			...
			if value in ['txt', 'code']:
				global_back_now='<div class="gb"><p>'+open(file,'r').read(256).replace('\n','</p><p>')+'</p></div>'
...
def html_render(path,fullpath, back, dirs, files, template_file=None):
	...
	for file in files:
		...
		if 'dir-info' in file_name:
			...
			continue
		if (file_name=='fileList.json')or(file_name=='fileList_sub.json'):
			continue
...
def crawl(path, fullpath,back=None, recursive=False, template_file=None):
	...
	for dir in dirs:
		crawl(dir,fullpath+[dir.split('/')[-1]], path, recursive, template_file)
